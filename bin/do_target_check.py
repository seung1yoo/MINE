...
class Do_targetcheck_grep:
    ...
    def make_cmds(self, app_bedtools, input_dic):
        ...
        for sam_num, f_type_dic in input_dic.iteritems():
            ...
            if self.mine.targetbed:
                ...
                opts.append('-a')
                opts.append(f_type_dic['call.bedGraph.sort'])
                opts.append('-b')
                opts.append(self.mine.targetbed)
                opts.append('|')
                opts.append('gzip')
                opts.append('-f')
                opts.append('>')
    ...
    def make_out_fn(self, sam_num):
        out_fn = os.path.join(self.outdir, sam_num,
            '{0}.call.bedGraph.final.gz'.format(sam_num))
        return out_fn
    ...
    def select_input(self):
        input_dic = dict()
        input_dic = self.eco.eco_dic['do_targetcheck_sort']
        return input_dic
