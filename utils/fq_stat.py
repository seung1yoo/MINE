...
class Read :
    ...
    def qual_stat(self,base):
        ...
        for q in self.qul :
            ...
            if qual>=20 :
                self.n_q20+=1
        self.mean_qual=1.0*_sum/len(self.qul)
        #
...
class ComposeStat :
    ...
    def to_report(self,out) :
        out.write("# GC-contents: NT_GC, GC%, GC%(excluse N)\n")
        out.write("GC\t%d\t%.2f\t%.2f\n"%(self.nt_gc,\
                100.0*self.nt_gc/(self.n_base-self.nt_nn),\
                100.0*self.nt_gc/(self.n_base)))
        out.write("# AT-contents: NT_AT, AT%, AT%(excluse N)\n")
        out.write("AT\t%d\t%.2f\t%.2f\n"%(self.nt_at,\
                100.0*self.nt_at/(self.n_base-self.nt_nn),\
                100.0*self.nt_gc/(self.n_base)))
        out.write("# 'N'%\n")
...
class ReadStat :
    ...
    def add_read(self,read):
        self.n_read+=1
        if read.mean_qual>=30.0 :
            self.n_q30_read+=1
        if read.mean_qual>=20.0 :
            self.n_q20_read+=1
    def to_report(self,out) :
        out.write("# N_READ\tN_READ_Q30\tN_READ_Q20\n")
        out.write("RD\t%d\t%d\t%d\n"%\
                (self.n_read,self.n_q30_read,self.n_q20_read))
...
class BaseStat :
    ...
    def add_read(self,read):
        self.n_base+=len(read.seq)
        self.n_q30_base+=read.n_q30
        self.n_q20_base+=read.n_q20
    def to_report(self,out) :
        out.write("# N_BASE\tN_BASE_Q30\tN_BASE_Q20\n")
        out.write("BS\t%d\t%d\t%d\n"%\
                (self.n_base,self.n_q30_base,self.n_q20_base))
...
def read_fastq(fn) :
    ...
    while True :
        ...
        if line=='' :
            break
        f=Read()
        f.title=line.strip().strip()
        f.seq=_in.readline().strip()
        f.cmt=_in.readline().strip()
        f.qul=_in.readline().strip()

        if not f.title.startswith("@"):
            sys.stderr.write("Wrong FASTQ\n")
            return
...
def main() :
    ...
    for in_fn in run.in_fn_s :
        q_base=select_qual_base(in_fn)
        for read in read_fastq(in_fn) :
            if len(read.seq)<run.min_length :
                continue
            read.qual_stat(q_base)
            ...
            buf.append("%.1f"%(read.mean_qual))
            buf.append("%d"%(read.n_q30))
            buf.append("%d"%(read.n_q30))
